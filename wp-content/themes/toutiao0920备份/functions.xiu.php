...
function hui_nav_menu($class='nav', $location='nav'){
    echo '<ul class="'.$class.'"><li class="menu-item menu-item-type-taxonomy menu-item-object-category menu-item" style="margin-left: 0px; margin-right: 0px;"><a href="/">推荐</a></li><li class="navmore"></li><li class="navmore"></li>'.str_replace("</ul></div>", "", preg_replace("/<div[^>]*><ul[^>]*>/", "", wp_nav_menu(array('theme_location' => $location, 'echo' => false)) )).'</ul>';
}
...
function hui_posts_related($title='', $limit=8, $model='thumb'){
    ...
    $i = 0;
    echo '<div class="relates'.' relates-model-'.$model.'">';//.'<div class="title"><span class="title_t">'.$title.'</span></div>';
    if ( $posttags ) {
        ...
        // };
        get_template_part( 'relate_excerpt' );
        wp_reset_query();
    }
    if ( $i < $limit ) {
        ...
        foreach($images as $src){
          ...
          /*  if( _hui('list_thumb_out') && _hui('list_thumb_out_text') ){
            ...
            if(
                ...
                ($etype == 'four' && $counter >= 3 && $i >= 3) ||
                ($counter >= 12 && $i >= 12) ||
                ($counter >= 8 && $counter < 12 && $i >= 8) ||
                ($counter >= 3 && $counter < 8 && $i >= 3) ||
                ($counter > 0 && $counter < 3 && $i >= 1)
            ){
                break;
            }
        }
