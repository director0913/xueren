...
function getPages($url){
	...
	for ($i=1; $i < 5; $i++) { 
		...
		foreach ($html->find('li.sex-lists-item a') as $k => $v) {

			$info['title'] = $v->innertext;
			$info['href'] = $v->href?'https://sex.fh21.com.cn'.$v->href:'';
			//var_dump($info['href']);die;
			if ($info['href']) {
...
function getInfo($url){
	$html = file_get_html($url);
	if ($html) {
		$info = $html->find('div.detail-content-box',0)->innertext ;
		return $info;
	}
	return '';
}
